...
def load_page(page_number):
    entities = []
    response = requests.get(f"https://api.alnair.ae/v1/rc/search?page={page_number}&mapBounds%5Beast%5D=55.55305480957031&mapBounds%5Bnorth%5D=25.41350860804229&mapBounds%5Bsouth%5D=24.89453374486885&mapBounds%5Bwest%5D=54.96871948242188&isList=1&isPin=1").json()
...
def load_page_with_sale_type(page_number, sale_type): #1, 2, 3
    entities = []
    response = requests.get(f"https://api.alnair.ae/v1/rc/search?page={page_number}&mapBounds%5Beast%5D=55.52490234375001&mapBounds%5Bnorth%5D=25.4538149753354&mapBounds%5Bsouth%5D=24.85466521467615&mapBounds%5Bwest%5D=54.99755859375001&saleType%5B0%5D={sale_type}&isList=1&isPin=1").json()
    for item in response["data"]["list"]:
        entities.append(item)
    return entities
...
def load_page_with_parking_spaces(page_number, parking_spaces):
    entities = []
    response = requests.get(f"https://api.alnair.ae/v1/rc/search?page={page_number}&mapBounds%5Beast%5D=55.52490234375001&mapBounds%5Bnorth%5D=25.4538149753354&mapBounds%5Bsouth%5D=24.85466521467615&mapBounds%5Bwest%5D=54.99755859375001&parkingSpaces={parking_spaces}&isList=1&isPin=1").json()
    for item in response["data"]["list"]:
        entities.append(item)
    return entities
...
def get_data_by_entity(entity):
        ...
        for item in units[unit]["items"]:
            ...
            for index in range(len(units_)):
                ...
                if unit_["num_bedrooms"] == num_bedrooms and unit_["size_min"] == size_min \
                    ...
                    psf_min = float(price_min) / float(size_min)
                    psf_max = float(price_max) / float(size_max)
                    units_[index] = {"unit_type": title if title is not None else unit_type, "id": unit_id,
                                     "General": name,
    ...
    return {"name": name, "ID": id, "address": address, "district": district, "description": description, "site": site,
            "longitude": longitude, "latitude": latitude, "developer": developer, "logo_url": logo_url,
            "logo_logo": logo_logo,
            "photo_url": photo_url, "photo_logo": photo_logo, "construction_percent": construction_percent,
            "construction_inspection_date": construction_inspection_date,
            "construction_percent_out_of_plan": construction_percent_out_of_plan,
            "transactions": transactions, "total": total, "apartments": apartments, "start_at": start_at,
            "planned_at": planned_at,
            "predicted_completion_at": predicted_completion_at, "completed_at": completed_at,
            "distance_to_city": distance_to_city,
            "brochure": brochure, "overall_available_units": overall_available_units, "album": album,
            "polygon": polygon,
            "facilities": residential_complex_advantages,
            "residential_complex_sale_status": residential_complex_sale_status,
...
if __name__ == "__main__":
    ...
    from platform_7s.models import Object_Dubai, Unit_Dubai

    Object_Dubai.objects.all().delete()
    Unit_Dubai.objects.all().delete()
